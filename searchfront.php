<?php
    ...
    if ($jsonData) {
        ...
        $searchResults = mysqli_query($con, "SELECT * FROM listingdata WHERE REPLACE(LOWER(stat), ' ', '') = REPLACE(LOWER('$state'), ' ', '') AND (LOWER(city) = LOWER('$city') OR LOWER(addy) LIKE '%$city%') AND expired = 'false' LIMIT 10;");
        ...
        if ($searchResults && mysqli_num_rows($searchResults) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($searchResults)) {
                $html .= '<div class="rounded-box" style="flex: 1 0 calc(33.33% - 20px); max-width: calc(20% - 20px); height: 500px; margin: 10px;" onclick="handleBoxClick()">';
                ...
                $html .= '</div>';
                $html .= '<p>Address: Click To View</p>';
